...
/* ── Merchant TX card ─────────────────────────────────────────── */
const MerchantCard = ({ tx, index, onView }) => (
  <motion.div
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      ...
      <p className="text-[10px] text-gray-400 font-mono truncate mb-1.5">{tx.transaction_id}</p>
...
const Merchant = () => {
  ...
  useEffect(() => {
    getAllMerchantTransactions()
      .then((res) => {
        ...
        else toast.error("Failed to fetch merchant transactions.");
      })
  ...
  const summaryStats = [
    ...
    { label: "Credited", value: count("completed") + count("credited"),      bg: "bg-emerald-50", border: "border-emerald-100", text: "text-emerald-600" },
    { label: "Pending",  value: count("pending"),                            bg: "bg-amber-50",   border: "border-amber-100",   text: "text-amber-600"   },
    { label: "System",   value: count("system"),                             bg: "bg-blue-50",    border: "border-blue-100",    text: "text-blue-600"    },
  ];

  const filterTabs = [
    { key: "all",       label: "All",     count: transactions.length,                       color: "border-gray-500"    },
    { key: "completed", label: "Credited",count: count("completed") + count("credited"),    color: "border-emerald-500" },
    { key: "pending",   label: "Pending", count: count("pending"),                          color: "border-amber-500"   },
    { key: "system",    label: "System",  count: count("system"),                           color: "border-blue-500"    },
  ];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <h1 className="text-xl font-extrabold text-gray-900">Merchant Transactions</h1>
        <p className="text-sm text-gray-400 mt-0.5">All merchant wallet and commission transactions</p>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <Toolbar search={search} onSearch={setSearch} count={filtered.length} loading={loading} placeholder="Search by name, TX ID or type…" />

        <div className="px-4 pb-4 flex flex-col gap-3">
          {loading
            ? Array.from({ length: 7 }).map((_, i) => <SkeletonCard key={i} />)
            : filtered.length === 0
              ? <EmptyState message={search ? "No results found." : "No merchant transactions yet."} icon={PiStorefrontBold} />
              : filtered.map((tx, i) => <MerchantCard key={tx.id} tx={tx} index={i} onView={setSelected} />)
          }
        </div>
      </div>
      ...
      {selected && (
        <DetailModal
          ...
          subtitle={selected.transaction_id}
          onClose={() => setSelected(null)}
        >
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Merchant</p>
            ...
            <div>
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-2xl px-4 py-1">
                <DetailRow label="TX ID"  value={selected.transaction_id} mono />
