...
const EditVendor = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res?.success) {
        setUser(res.data); setForm(res.data); setInitial(res.data);
      } else toast.error("Failed to fetch vendor");
    })();
  }, [uid]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      } else toast.error(res?.message || "Failed to update vendor");
  ...
  const initials = (user.full_name || user.username || "M").slice(0, 2).toUpperCase();
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-gray-800">Edit Merchant</p>
            <p className="text-xs text-gray-400 mt-0.5">Modify merchant account details and permissions</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 flex flex-wrap items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-orange-100 text-orange-700 font-bold text-lg flex items-center justify-center">
          {initials}
        </div>
        <div>
          ...
          <div className="flex items-center gap-2 mt-1.5">
            <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold bg-orange-100 text-orange-700">merchant</span>
