...
const Edit = ({ onClose, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updatePlatformById(existingData.id, fd);
      if (res?.success) {
        toast.success(res?.message || "Platform updated successfully");
        onClose?.();
      } else toast.error(res?.message || "Failed to update platform");
    } catch { toast.error("Error updating platform"); }
    setSubmitting(false);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-semibold text-gray-500">Platform Image <span className="font-normal text-gray-400">(optional — replace current)</span></label>
        {imagePreview && (
          <img src={imagePreview} alt="Current" className="h-16 w-16 rounded-xl object-cover border border-gray-100" />
        )}
        <label className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-gray-200 bg-gray-50 hover:bg-gray-100 cursor-pointer transition-all">
          ...
          <input type="file" accept=".png,.jpeg,.jpg" onChange={handleImageChange} className="hidden" />
        </label>
      </div>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button type="submit" disabled={submitting}
          ...
          <HiCheck size={12} /> {submitting ? "Updating..." : "Update Platform"}
        </button>
      </div>
    </form>
  );
