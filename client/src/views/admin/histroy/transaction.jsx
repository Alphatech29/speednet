...
const Transaction = () => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <ToastContainer />
      <h1 className="text-[18px] font-semibold text-zinc-700">Transaction History</h1>

      <div className="bg-white flex flex-col w-full border rounded-lg px-3 py-4">
        <div className="flex justify-between items-center gap-2 flex-wrap">
          <input
            className="rounded-md border px-2 py-1 md:w-[300px]"
            type="search"
        ...
        <div className="md:w-full min-h-[400px] flex flex-col gap-2 mt-4 text-gray-800 overflow-auto">
          {loading ? (
            ...
            <Table hoverable className="bg-transparent">
              <Table.Head className="bg-transparent text-gray-600 text-[13px]">
              ...
              <Table.Body className="divide-y">
                {filteredTransactions.length > 0 ? (
                  filteredTransactions.map((txn, index) => (
                    <Table.Row
                      key={txn.id || txn.transaction_no || index}
