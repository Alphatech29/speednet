...
const FIELDS = [
  ...
  { id: "onlinesim_api_key", label: "Smspool API Key",   sensitive: true },
  { id: "onlinesim_rate",    label: "Markup Rate (%)",   type: "number"  },
];
...
const SmsTab = () => {
  ...
  useEffect(() => {
    getWebSettings().then((res) => {
      ...
      } else toast.error("SMS settings not found");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(res?.data?.message || "SMS settings saved");
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <TabHeader title="SMS Service" subtitle="Configure Smspool API credentials and markup rate">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-50">
          <div className="w-8 h-8 rounded-xl bg-sky-50 flex items-center justify-center flex-shrink-0">
            <FaCommentSms size={15} className="text-sky-500" />
          </div>
          <div>
            <p className="text-sm font-bold text-gray-800">Smspool Integration</p>
            <p className="text-[11px] text-gray-400 mt-0.5">Virtual number and SMS verification service</p>
          </div>
        </div>
        <div className="p-5">
          <FieldGrid>
