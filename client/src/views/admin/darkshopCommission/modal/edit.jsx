...
const EditCommission = ({ onClose, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!commissionRate.toString().trim()) {
      toast.error("Please enter a commission rate"); return;
    }
    setSubmitting(true);
    try {
      const res = await updateCategoryCommissionAPI({
      ...
      if (res?.success) {
        ...
        onClose?.();
      } else toast.error(res?.message || "Failed to update commission");
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        ...
        <input type="text" value={categoryName} disabled
          className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-100 bg-gray-50 text-gray-500 cursor-not-allowed outline-none" />
      </div>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button type="submit" disabled={submitting}
          ...
          <HiCheck size={12} /> {submitting ? "Updating..." : "Update Commission"}
        </button>
      </div>
    </form>
  );
